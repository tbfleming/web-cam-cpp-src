...
vector<typename ScanlineEdge::Edge> getVoronoiEdges(int debugArg0, int debugArg1, PolygonSet& geometry, double angle)
{
    ...
    printf("g5: %d edges\n", edges.size());
    // for(auto& edge: edges)
    //     printf("    %d,%d,%d => %d,%d,%d\n", edge.point1.x, edge.point1.y, edge.point1.z, edge.point2.x, edge.point2.y, edge.point2.z);
    return edges;
} // getVoronoiEdges
...
void processSpan(double passDepth, double maxDepth, vector<vector<PointWithZ>>& result, vector<Edge>& span)
{
    ...
    while (true) {
        ...
        if (reverse) {
            ...
            {
                //printf("rrr %d,%d,%d => %d,%d,%d\n", it->point1.x, it->point1.y, it->point1.z + deltaZ, it->point2.x, it->point2.y, it->point2.z + deltaZ);
            ...
            for (auto& edge: span)
            {
                //printf("fff %d,%d,%d => %d,%d,%d\n", edge.point1.x, edge.point1.y, edge.point1.z + deltaZ, edge.point2.x, edge.point2.y, edge.point2.z + deltaZ);
                clipTopZ(passDepth, maxDepth, path, Edge{
    ...
    double**& resultPaths, int& resultNumPaths, int*& resultPathSizes)
{
    try {
        ...
        double maxDepth = 0; // not implemented

        printf("a cutterAngle=%f passDepth=%f maxDepth=%f\n", cutterAngle, passDepth, maxDepth);
